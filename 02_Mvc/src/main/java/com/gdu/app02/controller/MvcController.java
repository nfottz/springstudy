...
public class MvcController {
	...
	*/
	...
	public String detailMe(@RequestParam(value="name", required = false, defaultValue = "이름없음") String name,	// 요청 파라미터 name이 없으면 "이름없음"을 사용한다.
						   @RequestParam(value="age", required = false, defaultValue = "0") int age,				// 요청 파라미터 age가 없으면 "0"을 사용한다.
					   /* 
					   		@RequestParam 생락할 때
					   		
					   		(String name, 	요청 파라미터 name이 없으면 null이 저장된다.
					   		int age, 		요청 파라미터 age가 없으면 null은 int로 변환하려고 하기 때문에 오류가 발생한다.
					   		Model model)
					   */
						   Model model) {
	...
	*/
	...
	/*
	 	@GetMapping("/detail.gdu")
	 	public String detainGdu(@ModelAttribute(value="p") Person person){	// Model에 저장할 속성명을 p로 해주세요!
	 		return "mvc/detail";											// ${p.name}, ${p.age}로 확인할 수 있다.
	 	}
	*/
