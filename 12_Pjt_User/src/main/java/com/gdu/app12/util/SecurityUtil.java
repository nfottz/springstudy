...
public class SecurityUtil {
  ...
  // SHA-256 암호화하기
  /*
   1. 입력값을 256비트(32바이트)로 암호화 처리하는 해시 알고리즘
   2. 암호화는 가능하지만 복호화는 불가능하다.
   3. 암호화된 결과를 저장하기 위한 32바이트 byte 배열이 필요하다.
   4. 1바이트 -> 16진수로 변환해서 암호화된 문자열을 만든다.
      (1바이트는 8자리 2진수로 2자리 16진수가 된다.)
   5. 32바이트 -> 16진수로 변환하면 64글자가 생성된다.
      (DB 칼럼의 크기를 VARCHAR2(64 BYTE)로 설정한다.)
  ...
  */
  public String getSha256(String str) {
    ...
    for(int i = 0; i < b.length; i++) {
      sb.append(String.format("%2X", b[i]));  // %X : 16진수, %2X : 2자리의 16진수
    }
    return sb.toString();
  }
