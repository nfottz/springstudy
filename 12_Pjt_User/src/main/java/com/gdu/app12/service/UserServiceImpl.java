...
public class UserServiceImpl implements UserService {
  ...
  public Map<String, Object> verifyEmail(String email) {
    Map<String, Object> map = new HashMap<String, Object>();
    map.put("enableEmail", userMapper.selectUserByEmail(email) == null
                        && userMapper.selectSleepUserByEmail(email) == null 
  ...
  public Map<String, Object> sendAuthCode(String email) {
    ...
    // 사용자에게 메일 보내기
    javaMailUtil.sendJavaMail(email, "[서비스이름] 인증요청", "인증번호는 <strong>" + authCode + "</strong>입니다.");
  ...
  public void autoLogin(HttpServletRequest request, HttpServletResponse response) {
    ...
    /*
       ...
       1. 자동 로그인을 체크한 경우
         1) session의 id를 DB의 AUTOLOGIN_ID 칼럼에 저장한다.
           (중복이 없고, 다른 사람이 알기 어려운 정보를 이용해서 자동 로그인에서 사용할 ID를 결정한다.)
         2) 자동 로그인을 유지할 기간(ex.15일)을 AUTOLOGIN_EXPIRED_AT 칼럼에 저장한다.
    ...
    */
    ...
    if(chkAutoLogin != null) {
      ...
      // 현재 + 15일 : java.sql.Date 클래스를 이용하여 작업한다.
      // java.sql.Date 클래스는 타임스탬프를 이용해서 날짜를 생성한다.
      userDTO.setAutologinExpiredAt(new java.sql.Date(System.currentTimeMillis() + 3600 * 24 * 15 * 1000)); 
      ...
      cookie.setMaxAge(3600 * 24 * 15);         // 초 단위로 15일 지정
  ...
  public void logout(HttpServletRequest request, HttpServletResponse response) {
    ...
    // DB에 저장된 자동 로그인 정보 삭제하기
    HttpSession session = request.getSession();
    userMapper.deleteAutologin((String)session.getAttribute("loginId"));
