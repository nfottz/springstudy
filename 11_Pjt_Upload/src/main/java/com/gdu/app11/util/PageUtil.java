...
public class PageUtil {
	...
	private int pagePerBlock = 2;	// 한 블록에 표시할 페이지의 개수(임의로 정한다)
...
//		totalPage = (totalRecord + recordPerPage - 1) / recordPerPage;
	...
	public String getPagination(String path) {
		...
		// path에 ?가ㅏ 포함되어 있으면 이미 파라미터가 포함된 경로이므로 &를 붙여서 page 파라미터를 추가한다.
		if(path.contains("?")) path += "&";	// path = "/app09/employees/pagination.do?order=ASC&"
		else path += "?";					// path = "/app09/employees/pagination.do?"
