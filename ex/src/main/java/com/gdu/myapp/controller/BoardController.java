...
public class BoardController {
	...
	// 반환타입 String : 이동할 jsp 파일명을 반환한다.
	// 반환된 이름은 servlet-context.xml의 ViewResolver에 의해 rendering 된다. (prefix + 반환값 + suffix)
	public String index() {
		return "index";
	}
	...
	*/
	...
	/*
	@GetMapping("/detail.do")
	public void getDetail(BoardDTO board) {	// board.setBoardNo()와 board.setTitle()이 사용된다.
		System.out.println(board);	// BoardDTO 객체를 출력하면 BoardDTO 클래스의 toString()이 동작한다.
	}
	
	@PostMapping("/detail.do")
	public void postDetail(BoardDTO board) { // board.setBoardNo()와 board.setTitle()이 사용된다.
		System.out.println(board);	// BoardDTO 객체를 출력하면 BoardDTO 클래스의 toString()이 동작한다.
	}
	*/
	...
	*/
	...
	@GetMapping("/detail.do")
	public String getDetail(BoardDTO board) {	// 파라미터를 객체로 받으면 자동으로 Model에 저장이 된다! 속성명은 boardDTO로 저장된다.
		return "detail";
	}
