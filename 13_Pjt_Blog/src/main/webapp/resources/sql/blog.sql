...
CREATE TABLE COMMENT_T (
    ...
    CREATED_AT          DATE,
    BLOG_NO             NUMBER,         -- FK -- 하나의 댓글에 여러가지 댓글이 달릴 수 있다.
    MEMBER_NO           NUMBER          -- FK -- 회원만 댓글을 달 수 있다.
);
...
-- 써머노트 이미지 외래키 : 작성한 블로그가 삭제되면 이미지도 함께 삭제된다.
...
-- 댓글 외래키 (사용자) : 댓글 작성자가 삭제되면 댓글의 MEMBER_NO를 NULL 처리한다.
ALTER TABLE COMMENT_T
