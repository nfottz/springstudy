...
<script>
	...
	// 자바스크립트 파일(*.js)에서는 ${contextPath}(자바에 의해서 해석되는) 가 인식되지 않기 때문에
	...
	$(function (){
		$('#content').summernote({
			...
			callbacks: {
				onImageUpload: function(files) {					// summernote 편집기에 이미지를 올리면 해당 이미지의 정보는 함수 function의 매개변수 files로 전달된다.
					for(let i = 0; i < files.length; i++) {			// files 배열을 순회하면서 이미지를 하나씩 처리한다.
						var formData = new FormData();				// 가상 form을 (주로 ajax 처리에서 사용)
						formData.append('file', files[i]);			// 가상 form에 추가한 요소 파일 정보를 얻다.
						$.ajax({									// 가상 form에 저장된 이미지를 HDD에 저장하고 저장된 경로(매핑)와 이름을 받아오는 ajax
							contentType: false,						// ajax을 이용한 파일 첨부에서 사용
							processData: false,						// ajax을 이용한 파일 첨부에서 사용 이 두개는 약속처럼 생각
							type: 'post',
							...
							success: function(resData){			// resData = {"src": "/app13/imageLoad/abcdefg.jpg"}
								$('#content').summernote('insertImage', resData.src);	// content에 <img src="/app13/imageLoad/abcdefg.jpg"> 태그가 추가된다.
																						// content에 추가된 img 태그의 src 속성 값은 servlet-context.xml의 resources(자원등록방식)태그에 의해서 /storage/summernote/abcdefg.jpg로 처리된다.
							}
						})	// ajax
					}	// for
				}	// onImageUpload
